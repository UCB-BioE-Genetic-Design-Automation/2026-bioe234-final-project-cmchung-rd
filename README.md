...
| **Gemini API rate limits** | Even with a paid Gemini 2.5 Flash key, the API enforces per-minute rate limits (RPM/TPM). Each freeform protocol run consumes 1–2 requests (2 when self-correction is triggered). Rapid back-to-back testing — especially during a demo or group testing session — can trigger `429 RESOURCE_EXHAUSTED` errors and force a wait before the next run is possible. | Space out test runs by ~30–60 seconds. Caching generated scripts by description hash would reduce repeat API calls for the same prompt. |
